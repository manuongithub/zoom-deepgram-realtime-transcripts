...
void PocoHelper::initialize(std::string wsEndPoint, const std::map<std::string, std::string>& extraHeaders, const std::string& encoding, int sampleRate, int channels)
{
    ...
    try {
        ...
        // Create a SocketReactor for handling events
        reactor = new Poco::Net::SocketReactor();

        // Register the WebSocket for read events
        reactor->addEventHandler(*m_psock, Poco::Observer<PocoHelper, Poco::Net::ReadableNotification, const Poco::AutoPtr<Poco::Net::ReadableNotification>&>(*this, &PocoHelper::onSocketReadable));

        // Start the reactor in a separate thread
        Poco::Thread reactorThread;
        reactorThread.start(Poco::RunnableAdapter<PocoHelper>(*this, &PocoHelper::runReactor));
