    ...
    try {
        ...
        if (m_psock->impl() && m_psock->impl()->initialized()) {
            ...
            if (response.getStatus() == Poco::Net::HTTPResponse::HTTP_OK) {
                ...
                logStream.str("");
                logStream << "WebSocket connection opened, but authentication failed. HTTP Status Code: ";
                
                /* Log the response content when authentication fails
                if (response.getContentLength() > 0) {
                    std::istream& responseStream = response.stream();
                    std::string responseContent((std::istreambuf_iterator<char>(responseStream)), std::istreambuf_iterator<char>());
