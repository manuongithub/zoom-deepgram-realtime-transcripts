...
void PocoHelper::send_buffer(char* buffer, unsigned int bufferLen)
{
    ...
    try
    {
        ...
        if (msgSize <= 0 && isSocketReady)
        {
            ...
            Poco::StreamConverter::encode(std::istreambuf_iterator<char>(std::istringstream(buffer)), std::istreambuf_iterator<char>(),
                                          std::ostreambuf_iterator<char>(base64Encoded), Poco::Base64Encoder());
