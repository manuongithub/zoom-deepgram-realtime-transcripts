...
void PocoHelper::initialize(std::string wsEndPoint, const std::map<std::string, std::string>& extraHeaders, const std::string& encoding, int sampleRate, int channels)
{
    ...
    std::string queryString = "encoding=" + encoding + "&punctuate=true" + "&sample_rate=" + std::to_string(sampleRate) + "&channels=" + std::to_string(channels) + "&model=nova-2" + "&smart_format=true";
...
void PocoHelper::receive_buffer() {
    try {
        ...
        if (bytesRead > 0) {
            ...
            std::stringstream logStream;
            logStream << std::string(readMsg, bytesRead);
            Log::info(logStream.str());
        } else if (bytesRead == 0) {
