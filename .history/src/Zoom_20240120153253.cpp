...
SDKError Zoom::startRawRecording() {
    ...
    if (m_config.useRawAudio()) {
        ...
        if (!m_audioSource) {
            ...
            // Read and set deepgram-api-key from the config
            m_config.readDeepgramApiKeyFromConfig();
            auto deepgramApiKey = m_config.deepgramApiKey();
            if (deepgramApiKey.has_value()) {
                m_audioSource->setDeepgramApiKey(deepgramApiKey.value());
            }
        }
