...
void PocoHelper::receive_buffer() {
    try {
        ...
        if (bytesRead > 0) {
            ...
            Poco::Dynamic::Var result = parser.parse(readMsg);
            Poco::JSON::Object::Ptr json = result.extract<Poco::JSON::Object::Ptr>();

            // Check if 'is_final' field is present and true
            bool isFinal = false;
            if (json->has("is_final")) {
                isFinal = json->getValue<bool>("is_final");
            }

            // If 'is_final' is true, print the transcript
            if (isFinal) {
                Poco::JSON::Object::Ptr channel = json->getObject("channel");
                if (channel) {
                    Poco::JSON::Array::Ptr alternatives = channel->getArray("alternatives");
                    if (alternatives && alternatives->size() > 0) {
                        Poco::JSON::Object::Ptr firstAlternative = alternatives->getObject(0);
                        std::string transcript = firstAlternative->optValue<std::string>("transcript", "");
                        Log::info("Transcript: " + transcript);
                    } else {
                        Log::error("No alternatives array found.");
                    }
                } else {
                    Log::error("No channel object found.");
                }
            } else {
                Log::info("Received message, but 'is_final' is false.");
            }
        } else if (bytesRead == 0) {
