...
{
    Config config;
    config.readDeepgramApiKeyFromConfig();
    
    // Extracting the string from the optional
    if (auto apiKeyOptional = config.getDeepgramApiKey(); apiKeyOptional.has_value()) {
        m_dgApiKey = apiKeyOptional.value();
    } 
