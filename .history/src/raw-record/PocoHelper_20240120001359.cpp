...
void PocoHelper::initialize(std::string wsEndPoint, const std::map<std::string, std::string>& extraHeaders, const std::string& encoding, int sampleRate, int channels)
{
    ...
    try {
        ...
        // Run receive_buffer in a separate thread
        std::thread receiveThread(&PocoHelper::receive_buffer, this);
...
void PocoHelper::run_send_buffer_in_thread(char* buffer, unsigned int bufferLen) {
    std::thread sendThread(&PocoHelper::send_buffer, this, buffer, bufferLen);
    sendThread.detach();  // Detach the thread to allow it to run independently.
}

void PocoHelper::run_receive_buffer_in_thread() {
    std::thread receiveThread(&PocoHelper::receive_buffer, this);
    receiveThread.detach();  // Detach the thread to allow it to run independently.
}
