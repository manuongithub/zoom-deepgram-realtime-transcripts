...
void PocoHelper::initialize(std::string wsEndPoint, const std::map<std::string, std::string>& extraHeaders, const std::string& encoding, int sampleRate, int channels)
{
    ...
    std::string queryString = "encoding=" + encoding + "&tier=Nova" + "&smart_format=true" + "&sample_rate=" + std::to_string(sampleRate) + "&channels=" + std::to_string(channels);
    ...
    container = new Poco::Buffer<char>(4000);
...
void PocoHelper::send_buffer(char* buffer, unsigned int bufferLen)
{
    ...
    try
    {
        ...
        if (msgSize <= 0 && isSocketReady)
        {
            ...
            // Use std::ostringstream as an output stream for Base64Encoder
            ...
            base64Encoded = encodedStream.str();
