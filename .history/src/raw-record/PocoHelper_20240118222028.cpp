    ...
    : logHelper(Poco::Logger::get("TestLogger"))
{
    try
    {
        uri = std::make_unique<Poco::URI>(wsEndPoint);
        ...
        cs = std::make_unique<Poco::Net::HTTPSClientSession>(uri->getHost(), 443);

        // Disable SSL verification
        cs->set_verify_mode(Poco::Net::SSLManager::VERIFY_NONE);
        cs->set_verify_callback([](const bool, Poco::Net::X509Certificate) { return true; });

        request = std::make_unique<Poco::Net::HTTPRequest>(Poco::Net::HTTPRequest::HTTP_GET, uri->getPathEtc(), Poco::Net::HTTPMessage::HTTP_1_1);
        ...
        m_psock = std::make_unique<Poco::Net::WebSocket>(*cs, *request, response);

        m_psock->setBlocking(false);
        m_psock->setReceiveTimeout(Poco::Timespan(3, 0));
        m_psock->setSendTimeout(Poco::Timespan(3, 0));
        container = std::make_unique<Poco::Buffer<char>>(msgSize);
    ...
    {
        logHelper.error("Exception in constructor: " + ex.displayText());
        // Handle the exception or rethrow if appropriate
    }
}
...
PocoHelper::~PocoHelper()
{
    ...
    {
        logHelper.error("Exception in destructor: " + ex.displayText());
    }
}
