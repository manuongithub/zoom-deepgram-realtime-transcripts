...
void PocoHelper::initialize(std::string wsEndPoint, const std::map<std::string, std::string>& extraHeaders, const std::string& encoding, int sampleRate, int channels)
{
    ...
    try {
        ...
        reactor->addEventHandler(*m_psock, Poco::Observer<PocoHelper, Poco::Net::ReadableNotification>(Poco::Delegate<PocoHelper, Poco::Net::ReadableNotification>(this, &PocoHelper::onSocketReadable)));
