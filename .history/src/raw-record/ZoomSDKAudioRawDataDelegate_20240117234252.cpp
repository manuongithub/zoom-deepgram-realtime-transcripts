...
void ZoomSDKAudioRawDataDelegate::onMixedAudioRawDataReceived(AudioRawData *data) {
    ...
    std::string dgApiKey = "YOUR_DG_API_KEY";

    if (!deepgramWebSocketURL.empty()) {
        std::map<std::string, std::string> extraHeaders = {
            {"Authorization", "Token " + dgApiKey}
        };

        PocoHelper pocoHelper(deepgramWebSocketURL, extraHeaders);
        pocoHelper.send_buffer(data->buffer, data->size);
    }
}
